...
class PointCodingTests: XCTestCase {

    func testCoding() {
        let coordinate = Coordinate(latitude: 44.7866, longitude: 20.4489)
        let value = Point(title: "XXX", coordinate: coordinate)
        guard let encoded = value.toJSON() else {
            XCTFail(); return
        }
        XCTAssertTrue(JSONSerialization.isValidJSONObject(encoded))
        let decoded = Point(json: encoded)
        XCTAssertEqual(value, decoded)
    }

    static var allTests : [(String, (PointCodingTests) -> () throws -> Void)] {
        return [
