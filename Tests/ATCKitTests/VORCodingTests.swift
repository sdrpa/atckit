...
class VORCodingTests: XCTestCase {

    func testCoding() {
        let coordinate = Coordinate(latitude: 44.7866, longitude: 20.4489)
        let value = VOR(title: "XXX", coordinate: coordinate, frequency: 121.5)
        guard let encoded = value.toJSON() else {
            XCTFail(); return
        }
        XCTAssertTrue(JSONSerialization.isValidJSONObject(encoded))
        let decoded = VOR(json: encoded)
        XCTAssertEqual(value, decoded)
    }

    static var allTests : [(String, (VORCodingTests) -> () throws -> Void)] {
        return [
